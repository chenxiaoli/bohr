    ...
    def get_prev_timestamp(self, timpstamp, bin_size):
        ...
        if bin_size == "1M":
            if timpstamp.month == 1:
                start_time = timpstamp.replace(year=timpstamp.year - 1, month=12, day=1, hour=0, minute=0, second=0,
                                               microsecond=0)
            else:
                start_time = timpstamp.replace(month=timpstamp.month - 1, day=1, hour=0, minute=0, second=0,
                                               microsecond=0)
        elif bin_size == "1y":
            start_time = timpstamp.replace(year=timpstamp.year - 1, month=1, day=1, hour=0, minute=0, second=0,
                                           microsecond=0)
        elif bin_size == "1w":
...
class TradeBucketed(object):
    ...
    def get_trade_bucketed_first_one(self, bin_size):
        cursor = self.collection.find({"symbol": self.symbol, "binSize": bin_size}).sort("timestamp", 1).limit(1)
        for doc in cursor:
            doc["timestamp"] = doc["timestamp"].replace(tzinfo=pytz.utc)
            return doc
    ...
    def _create_first_time(self, bin_size, bin_size_params):
        ...
        first = self.get_trade_bucketed_first_one(bin_size="1m")
    ...
    def _create_next_timestamp(self, start_time, bin_size, bin_size_params):
        ...
        if bin_size == "1M":
            if start_time.month == 12:
                ...
                start_time = start_time.replace(month=start_time.month + 1, day=1, hour=0, minute=0, second=0,
                                                microsecond=0)
        elif bin_size == "1y":
            start_time = start_time.replace(year=start_time.year + 1, month=1, day=1, hour=0, minute=0, second=0,
                                            microsecond=0)
    ...
    def _increase_create_trade_bucketed(self, bin_size, bin_size_params):
        ...
        if orgin_bin_last_one_time >= end_time and not first:
            start_time = self._create_next_timestamp(start_time=start_time, bin_size=bin_size,
                                                     bin_size_params=bin_size_params)
            end_time = self._end_time(start_time, bin_size, bin_size_params)
        ...
        for item in self.collection.find(
                {"symbol": self.symbol, "binSize": bin_size, "timestamp": {"$gte": start_time, "$lt": end_time}}).sort(
            [("timestamp", 1)]):
            items.append(item)
